    ...
    (function(Highcharts) {
            ...
            try {
                ...
                if (!webKit && nav.userAgent.toLowerCase().indexOf('firefox') < 0) {
                    return domurl.createObjectURL(new win.Blob([svg], {
                        type: '/static/image/svg+xml;charset-utf-16'
                    }));
                }
            } catch (e) {
                // Ignore
            }
            return 'data:/static/image/svg+xml;charset=UTF-8,' + encodeURIComponent(svg);
        };
        ...
        Highcharts.downloadSVGLocal = function(svg, options, failCallback, successCallback) {
            var svgurl,
                ...
                imageType = options.type || '/static/image/png',
                filename = (options.filename || 'chart') + '.' + (imageType === '/static/image/svg+xml' ? 'svg' : imageType.split('/')[1]),
            ...
            if (imageType === '/static/image/svg+xml') {
                // SVG download. In this case, we want to use Microsoft specific Blob if available
                try {
                    if (nav.msSaveOrOpenBlob) {
                        blob = new MSBlobBuilder();
                        blob.append(svg);
                        svgurl = blob.getBlob('/static/image/svg+xml');
                    } else {
        ...
        Highcharts.Chart.prototype.getSVGForLocalExport = function(options, chartOptions, failCallback, successCallback) {
            ...
            try {
                ...
                for (i = 0, l = images.length; i < l; ++i) {
                    el = images[i];
                    Highcharts.imageToDataUrl(el.getAttributeNS('http://www.w3.org/1999/xlink', 'href'), '/static/image/png', {
                            imageElement: el
                        }, options.scale,
                        embeddedSuccess,
                        // Tainted canvas
        ...
        Highcharts.Chart.prototype.exportChartLocal = function(exportingOptions, chartOptions) {
            var chart = this,
                ...
                svgSuccess = function(svg) {
                    ...
                    if (
                        ...
                        options.type !== '/static/image/svg+xml'
                    ) {
            ...
            if (
                (isMSBrowser && options.type !== '/static/image/svg+xml' ||
        ...
        merge(true, Highcharts.getOptions().exporting, {
            libURL: 'https://code.highcharts.com/5.0.6///static/lib/',
            buttons: {
                contextButton: {
                    menuItems: [{
                        textKey: 'printChart',
